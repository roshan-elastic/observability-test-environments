...
func main() {
	...
	if _, err := os.Stat(embedFolder); os.IsNotExist(err) {
		logger.Fatal("Configs directory does not exists!")
	}
	...
	err := filepath.Walk(embedFolder, func(path string, info os.FileInfo, err error) error {
		...
		if info.IsDir() {
			// Skip directories
			logger.Infof(path, "is a directory, skipping...")
			return nil
		} else {
			// If element is a simple file, embed
			logger.Infof(path, "is a file, packing in...")

			b, err := os.ReadFile(path)
			if err != nil {
				// If file not reading
				logger.Infof("Error reading %s: %s", path, err)
				return err
			}
	...
	if err != nil {
		logger.Fatal("Error walking through embed directory:", err)
	}

	// Create blob file
	f, err := os.Create(blobFileName)
	if err != nil {
		logger.Fatal("Error creating blob file:", err)
	}
	defer f.Close()
	...
	if err = tmpl.Execute(builder, configs); err != nil {
		logger.Fatal("Error executing template", err)
	}

	// Formatting generated code
	data, err := format.Source(builder.Bytes())
	if err != nil {
		logger.Fatal("Error formatting generated code", err)
	}

	// Writing blob file
	if err = os.WriteFile(blobFileName, data, os.ModePerm); err != nil {
		logger.Fatal("Error writing blob file", err)
	}
}
