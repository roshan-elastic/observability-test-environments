...
func TestShowDestroyClusterForm(t *testing.T) {
	...
	// Assuming ShowDestroyClusterForm takes a slack.Client and returns an error
	err := ShowDestroyClusterForm(slackClient, "trigger123", "user123")
	assert.NoError(t, err, "ShowDestroyClusterForm should not return an error")
}
