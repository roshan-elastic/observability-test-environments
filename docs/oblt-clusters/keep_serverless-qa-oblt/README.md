...
Keep_serverless-qa-oblt is an observability test cluster deployed using the latest serverless version deployed in the [QA environment][].
The aim of keep_serverless-qa-oblt is to evaluate the observability features in a serverless environment.
The deployment is made following the instructions at [create a QA deployment][]
The keep_serverless-qa-oblt deployment has two pieces, one is the Elastic Stack and a Kubernetes(k8s) cluster.

The Elastic Stack is deployed on the [QA environment][],
the deployment uses the Elastic Stack available on the [QA environment][] at the time of the creation,
...
```bash
oblt-cli cluster secrets credentials --cluster-name keep_serverless-qa-oblt
```

## Update

The update of the clusters is automatic when the [QA environment][] push a new version of the stack.
The version to update is the latest stable version of the latest on the [QA environment][].

[QA environment]: https://docs.elastic.dev/serverless/qa
[create a QA deployment]: https://docs.elastic.dev/serverless/create-project
