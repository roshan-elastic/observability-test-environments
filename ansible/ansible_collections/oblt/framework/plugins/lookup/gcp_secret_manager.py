...
# pending to merge:
# https://github.com/ansible-collections/google.cloud/pull/578
# flake8: noqa
# noqa
...
DOCUMENTATION = '''
    ...
    name: oblt.framework.gcp_secret_manager
...
- name: Test secret using env variables for credentials
  ansible.builtin.debug:
    msg: "{{ lookup('google.cloud.oblt.framework.gcp_secret_manager', key='secret_key') }}"

- name: Test secret using explicit credentials
  ansible.builtin.debug:
    msg: "{{ lookup('google.cloud.oblt.framework.gcp_secret_manager', key='secret_key', project='project', auth_kind='serviceaccount', service_account_file='file.json') }}"

- name: Test getting specific version of a secret (old version)
  ansible.builtin.debug:
    msg: "{{ lookup('google.cloud.oblt.framework.gcp_secret_manager', key='secret_key', version='1') }}"

- name: Test getting specific version of a secret (new version)
  ansible.builtin.debug:
    msg: "{{ lookup('google.cloud.oblt.framework.gcp_secret_manager', key='secret_key', version='2') }}"
'''
