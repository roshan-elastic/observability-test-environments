...
This README is intended to be a quick start guide for getting up and running quickly. For
...
There are two typical approaches to managing terraform resources in CI:

1. Define a test utility in your programming language, to start a terraform process to init and apply a template to start Azure resources for tests, using the environment variables set in CI to authenticate with a Service Principal, and destroy resources when no longer required. Defining the test utility in the language allows control over what resources are deployed and when, and allows authenticating with local credentials when running tests locally, and with Service Principal credentials when running in CI. [An example implementation can be found in the .NET APM agent](https://github.com/elastic/apm-agent-dotnet/blob/36a7cafbd8e257eace6caee99f5785637c0e2ca1/test/Elastic.Apm.Azure.Storage.Tests/AzureStorageTestEnvironment.cs).
2. Define a step in the Jenkinsfile to deploy Azure resources using terraform templates, exposing outputs as environment variables, for tests to use.

### Resource group naming and cleanup

An environment variable can be set in CI to define a resource group name prefix for any resource groups deployed as part of a test run. [An example can be found in the .NET agent Jenkinsfile](https://github.com/elastic/apm-agent-dotnet/blob/36a7cafbd8e257eace6caee99f5785637c0e2ca1/Jenkinsfile#L19). The test utility can uses this when deploying resources with terraform.

[A Jenkins step can delete and resource groups created as part of the test run, using the group name prefix to target those resource groups](https://github.com/elastic/apm-agent-dotnet/blob/36a7cafbd8e257eace6caee99f5785637c0e2ca1/Jenkinsfile#L554-L575).
